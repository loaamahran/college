...
try{
    ...
    {message:'Please enter Email,Password'})} // lw mfesh mail aw password
db.query('select * from student where email=?',[email],async(error,results)=>{
console.log(results);
if(results.length==0)//email is already registred
{return res.render('login',{
message:'That Email or ID not in use'});
}
if(!results||!await bcrypt.compare(password,results[0].password))//not find users in database or wrong password
{
...
db.query('insert into student set ?',{name:name,email:email,password:hashedPassword,id:id},(error,results)=>{
if(error){
console.log(error);    
}
else{res.render('index',{message:'User registered'})}
...
exports.profileData= async(req,res,next)=>{
    ...
    console.log(decoded);// will show user id when token make and when expire
    //2) check if user still exist
    db.query('select * from student_profile where id=?',[decoded.id],(error,result)=>{
    console.log(result);
    if(!result){
     return next();   
    }
